...
def write_subsequences_group(y, sr, starts, m_secs, timestep, output_dir):
...
def write_all_sequence_audio(audio_path, all_seqs, m_secs, timestep, output_dir):
    y, sr = librosa.load(audio_path)
    for i, seq in enumerate(all_seqs):
        out_dir = os.path.join(output_dir, f'motif_{i}/')
        if isinstance(m_secs, (list, np.ndarray)):
            l = m_secs[i]
        else:
            l = m_secs
        write_subsequences_group(y, sr, seq, l*timestep, timestep, out_dir)
